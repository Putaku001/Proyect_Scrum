...
<head>
  ...
  <style>
    ...
    body {
      ...
      color: #f0f0f0; display: flex; justify-content: center; padding: 40px 20px;
    }
    .profile-box {
      background: var(--panel); padding: 30px; border-radius: 12px;
      max-width: 500px; width: 100%; box-shadow: 0 4px 12px rgba(0,0,0,.5);
    }
    .profile-box h2 {
      text-align: center; color: var(--primary); margin-bottom: 20px;
    }
    .profile-box label {
      display: block; margin-top: 15px; font-weight: bold; color: #ccc;
    }
    .profile-box input {
      width: 100%; padding: 12px; margin-top: 8px; border: none;
      border-radius: 8px; background: #3b3e5c; color: #f0f0f0;
    }
    .guardar-btn {
      background-color: var(--primary); color: #000; font-weight: bold;
      border: none; border-radius: 8px; padding: 12px; cursor: pointer;
      font-size: 1em; width: 100%; margin-top: 20px; transition: background .3s;
    }
    .guardar-btn:hover {
      background-color: #00aacc;
    }
    .salir-btn {
      background: #3b3e5c; color: #fff; border: 1px solid #666;
    }
    .salir-btn:hover {
      background: #555;
    }
    .avatar-preview {
      display: block; margin: 0 auto 20px; border-radius: 50%;
    ...
    .badge {
      ...
      font-weight: bold; font-size: .9em; margin-bottom: 5px;
    }
    .badge.yes { background: #00d4ff; color: #000; }
    .badge.no  { background: #ff7676; color: #000; }
    ...
    .modal-content {
      background: #2b2e45; width: 90%; max-width: 680px; max-height: 90vh;
    ...
    .modal-grid {
      display: grid; grid-template-columns: repeat(auto-fill, 80px); gap: 16px;
    }
    .modal-grid img {
      width: 80px; height: 80px; border-radius: 50%; object-fit: cover;
      cursor: pointer; transition: transform .2s, border .2s;
    }
    .modal-grid img:hover { transform: scale(1.08); }
    .modal-grid img.selected { border: 3px solid var(--primary); }
    .modal-actions {
    ...
    .btn.cancel {
      background: none; color: #f66; border: 1px solid #f66;
    }
    .btn.save {
