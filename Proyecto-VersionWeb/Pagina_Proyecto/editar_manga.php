...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['titulo']) && !isset($_POST['accion_tomo'])) {
    ...
    sqlsrv_query($conn, "UPDATE Mangas SET
...
<head>
  ...
  <style>
    ...
    body {
      ...
      color: #f3f5fc; font-family: 'Segoe UI',Arial,sans-serif; min-height:100vh;
    }
    .container-main {
      max-width: 750px; margin: 32px auto 0; padding: 30px 25px 16px;
      background: #232544; border-radius: 22px; box-shadow: 0 6px 40px #000a;
      display:flex; flex-wrap:wrap; gap:32px 38px; justify-content:space-between;
    }
    .col-data { width:320px; min-width:260px; }
    .col-portada {flex:1; min-width:210px;}
    @media (max-width:900px){.container-main{flex-direction:column;gap:20px;}.col-data,.col-portada{width:100%;}}
    h1 { text-align: left; color: #0ed6e7; letter-spacing: 1px; font-size:2em; margin-bottom:15px;}
    .form-row {margin-bottom:17px;}
    label { font-weight: bold; display:block; margin-bottom:3px; font-size:1.04em;}
    input, select, textarea {
      width: 100%; padding: 10px 10px; border-radius: 9px;
      border: none; background: #1b1c2a; color: #fff; font-size: 1.08em;
      transition: outline .17s, box-shadow .17s;
    }
    input:focus, select:focus, textarea:focus { outline:2px solid #00eaff; box-shadow:0 0 0 2px #00eaff55; }
    input[type="file"] { background: transparent; color: #00eaff; }
    .portada-box {background:#18192b; border-radius:11px; padding:16px 12px 15px; box-shadow:0 1px 10px #0003;}
    .portada-box img { display:block; max-width:100%; max-height:320px; margin: auto; border-radius:10px; box-shadow:0 2px 17px #0028;}
    .form-actions {
      margin-top: 26px; display: flex; gap: 20px; justify-content:space-between;
    }
    .form-actions button {
      padding: 11px 0; border-radius: 11px; font-weight: bold; width:48%;
      cursor: pointer; border: none; background: #0ed6e7; color: #151a23;
      font-size: 1.1em; box-shadow: 0 2px 12px #0012;
      transition: background .13s, transform .10s;
    }
    .form-actions button:active { transform:scale(.97);}
    .form-actions button:hover { background: #03b8d2; }

    .seccion-tomos {max-width:750px; margin:42px auto 0;}
    .tomos-card {
      background:#18192b; border-radius: 18px; box-shadow: 0 2px 14px #0009;
      padding: 22px 24px 18px; margin-bottom: 16px;
    }
    .tomos-title { color:#0ed6e7; font-size:1.32em; margin-bottom:19px; letter-spacing:.7px;}
    .tomo {
      display: flex; align-items: center; justify-content: space-between; gap:14px;
      padding: 12px 0; border-bottom:1px solid #29304b;
    }
    .tomo:last-child {border-bottom:none;}
    .tomo-info {
...
.tomo-premium-badge.no-premium {
  ...
  color: #0b5d7d;
    ...
    .tomo-name {
      ...
      font-size:1.08em; letter-spacing:.2px; cursor:pointer;
    }
    .tomo-name[title]:hover {text-decoration:underline;}
    .tomo-actions form {display:inline;}
    .tomo-actions button {
      font-size: 1.09em; padding: 7px 12px; border-radius: 8px;
      border: none; cursor: pointer; font-weight: 600; margin-left: 7px;
      background: #0ed6e7; color:#191c24; transition: background .11s;
    }
    .tomo-actions button[title="Eliminar"] { background: #e13a42; color: #fff; }
    .tomo-actions button[title="Eliminar"]:hover { background: #c91c26; }
    .tomo-actions button[title="Marcar Premium"] { background: #ffe381; color: #a17d08; }
    .tomo-actions button[title="Marcar Premium"]:hover { background: #ffce38; }
    .tomo-actions button[title="Quitar Premium"] { background: #5feec2; color: #0d4233; }
    .tomo-actions button[title="Quitar Premium"]:hover { background: #16d49e; }
    .nuevo-tomo { margin-top:24px; background:#222641; padding:14px 13px 9px; border-radius:13px; box-shadow: 0 1px 10px #0003; }

    @media (max-width:600px) {
      .container-main, .seccion-tomos {max-width:99vw; padding:2vw;}
      .tomos-card {padding:12px 6vw;}
      .tomo-name {max-width:105px;}
      .portada-box {padding:10px;}
    }
